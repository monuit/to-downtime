...
export function MapView({ disruptions, selectedDistrict, onDistrictSelect, onDisruptionSelect }: MapViewProps) {
  ...
  useEffect(() => {
    ...
    const map = L.map(mapContainerRef.current).setView([43.6532, -79.3832], 11);
  ...
  useEffect(() => {
    ...
    // Always show individual disruption pins with clustering
    const filteredDisruptions = selectedDistrict 
    ...
    filteredDisruptions.forEach(disruption => {
      if (!disruption.coordinates) return;
      ...
      const marker = L.marker([disruption.coordinates.lat, disruption.coordinates.lng], { icon });
  ...
  }, [disruptions, selectedDistrict, onDistrictSelect, onDisruptionSelect]);
